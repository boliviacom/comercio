...
const TABLES_ALLOWING_CREATE = ['departamento'];

export class AdminDepartamentoManager {
    ...
    constructor(displayElementId, modalId = 'crud-modal') {
        ...
        this.currentTable = 'departamento';
        this.currentLinkText = 'Departamentos';
    ...
    async loadTable() {
        ...
        try {
            ...
            console.error('Error al cargar datos de departamento:', e);
    ...
    _renderSearchBox(tableName) {
        const searchInstructions = 'Busca por Nombre de Departamento';
        return `
    ...
    async showForm(tableName, action, id = null) {
        ...
        const titleText = action === 'create' ? 'Nuevo Departamento' : 'Editar Departamento';
    ...
    async handleFormSubmit(tableName, action, id = null) {
        ...
        if (!nombre) {
            alert('El nombre del departamento es obligatorio.'); return;
        }
    ...
    async toggleVisibility(id, isVisible) {
        ...
        try {
            ...
            alert(`Departamento eliminado correctamente. Recargando tabla...`);
            this.loadTable();
        } catch (e) {
